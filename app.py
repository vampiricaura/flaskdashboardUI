...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2[['Category', 'App', 'Reviews', 'Rating']].groupby(['Category', 'App'])\
            .agg({'Reviews': 'sum','Rating': 'mean'}).sort_values('Reviews', ascending = False).reset_index()\
            .head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    # Mengganti satuan angka Install agar lebih singkat
    ...
    plt.axis('off')
    plt.savefig('tree_maps.png',bbox_inches="tight")
